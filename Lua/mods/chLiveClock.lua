...
local function _format_time(t)
    local _msPart = string.sub(tostring(t), -3)
    local _secPartFull = tonumber(tostring(t):match'(.*)%..*')
    ...
    elseif _chameleon[0] == chamStates.Gameplay then
        if InfosDisplay[0].LiveClock == true then
            ...
            if _lClockStart then
		        _lTime = tostring(socket.gettime() - _lClockStart):match'(%d*%.%d%d)'
                if _lTime then
            ...
            local screenx, screeny = nRes(33), nRes(34)
            if _lClock then
